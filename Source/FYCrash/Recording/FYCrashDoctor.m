...
@interface  FYCrashDoctorParam: NSObject
...
@implementation FYCrashDoctorParam
...
@interface FYCrashDoctorFunctionCall: NSObject
...
@implementation FYCrashDoctorFunctionCall
...
- (NSString*) descriptionForObjCCall
{
    ...
    FYCrashDoctorParam* receiverParam = [self.params objectAtIndex:0];
    ...
    FYCrashDoctorParam* selectorParam = [self.params objectAtIndex:1];
    if(![selectorParam.type isEqualToString:@FYCrashMemType_String])
    {
        return nil;
    }
    ...
    for(int paramNum = 0; paramNum < paramCount; paramNum++)
    {
        ...
        if(paramNum < 2)
        {
            FYCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)paramNum + 2];
            if(param.value != nil)
            {
                if([param.type isEqualToString:@FYCrashMemType_String])
                {
...
- (NSString*) descriptionWithParamCount:(int) paramCount
{
    ...
    {
        FYCrashDoctorParam* param = [self.params objectAtIndex:(NSUInteger)i];
...
{
    return [[self crashReport:report] objectForKey:@FYCrashField_Error];
}
...
- (NSDictionary*) crashedThreadReport:(NSDictionary*) report
{
    ...
    NSDictionary* crashedThread = [crashReport objectForKey:@FYCrashField_CrashedThread];
    if(crashedThread != nil)
    {
        return crashedThread;
    }

    for(NSDictionary* thread in [crashReport objectForKey:@FYCrashField_Threads])
    {
        if([[thread objectForKey:@FYCrashField_Crashed] boolValue])
        {
            return thread;
        }
    }
...
{
    NSDictionary* backtrace = [threadReport objectForKey:@FYCrashField_Backtrace];
    return [backtrace objectForKey:@FYCrashField_Contents];
}

- (NSDictionary*) basicRegistersFromThreadReport:(NSDictionary*) threadReport
{
    NSDictionary* registers = [threadReport objectForKey:@FYCrashField_Registers];
    NSDictionary* basic = [registers objectForKey:@FYCrashField_Basic];
    return basic;
}
...
- (NSDictionary*) lastInAppStackEntry:(NSDictionary*) report
{
    ...
    {
        NSString* objectName = [entry objectForKey:@FYCrashField_ObjectName];
...
{
    NSDictionary* machError = [errorReport objectForKey:@FYCrashField_Mach];
    if(machError != nil)
    {
        NSString* exceptionName = [machError objectForKey:@FYCrashField_ExceptionName];
        return [exceptionName isEqualToString:@"EXC_BAD_ACCESS"];
    }
    NSDictionary* signal = [errorReport objectForKey:@FYCrashField_Signal];
    NSString* sigName = [signal objectForKey:@FYCrashField_Name];
...
{
    NSDictionary* crashedThread = [self crashedThreadReport:report];
    NSArray* notableAddresses = [crashedThread objectForKey:@FYCrashField_NotableAddresses];
    for(NSDictionary* address in [notableAddresses objectEnumerator])
    {
        NSString* type = [address objectForKey:@FYCrashField_Type];
        if([type isEqualToString:@"string"])
        {
            NSString* value = [address objectForKey:@FYCrashField_Value];
    ...
    {
        NSString* objectName = [entry objectForKey:@FYCrashField_ObjectName];
        NSString* symbolName = [entry objectForKey:@FYCrashField_SymbolName];
...
- (FYCrashDoctorFunctionCall*) lastFunctionCall:(NSDictionary*) report
{
    FYCrashDoctorFunctionCall* function = [[FYCrashDoctorFunctionCall alloc] init];
    NSDictionary* lastStackEntry = [self lastStackEntry:report];
    function.name = [lastStackEntry objectForKey:@FYCrashField_SymbolName];
    ...
    {
        FYCrashDoctorParam* param = [[FYCrashDoctorParam alloc] init];
        ...
        {
            param.type = [notableAddress objectForKey:@FYCrashField_Type];
            NSString* className = [notableAddress objectForKey:@FYCrashField_Class];
            NSString* previousClass = [notableAddress objectForKey:@FYCrashField_LastDeallocObject];
            NSString* value = [notableAddress objectForKey:@FYCrashField_Value];

            if([param.type isEqualToString:@FYCrashMemType_String])
            {
                param.value = value;
            }
            else if([param.type isEqualToString:@FYCrashMemType_Object])
            {
                param.className = className;
                param.isInstance = YES;
            }
            else if([param.type isEqualToString:@FYCrashMemType_Class])
            {
                param.className = className;
                param.isInstance = NO;
            }
...
- (NSString*) zombieCall:(FYCrashDoctorFunctionCall*) functionCall
{
...
{
    NSDictionary* stack = [crashedThreadReport objectForKey:@FYCrashField_Stack];
    return [[stack objectForKey:@FYCrashField_Overflow] boolValue];
}

- (BOOL) isDeadlock:(NSDictionary*) report
{
    NSDictionary* errorReport = [self errorReport:report];
    NSString* crashType = [errorReport objectForKey:@FYCrashField_Type];
    return [@FYCrashExcType_Deadlock isEqualToString:crashType];
}
...
- (NSString*) diagnoseCrash:(NSDictionary*) report
{
    @try
    {
        NSString* lastFunctionName = [[self lastInAppStackEntry:report] objectForKey:@FYCrashField_SymbolName];
        ...
        NSString* crashType = [errorReport objectForKey:@FYCrashField_Type];
        if([crashType isEqualToString:@FYCrashExcType_NSException])
        {
            NSDictionary* exception = [errorReport objectForKey:@FYCrashField_NSException];
            NSString* name = [exception objectForKey:@FYCrashField_Name];
            NSString* reason = [exception objectForKey:@FYCrashField_Reason]? [exception objectForKey:@FYCrashField_Reason]:[errorReport objectForKey:@FYCrashField_Reason];
        ...
        FYCrashDoctorFunctionCall* functionCall = [self lastFunctionCall:report];
        ...
        {
            uintptr_t address = (uintptr_t)[[errorReport objectForKey:@FYCrashField_Address] unsignedLongLongValue];
            if(address == 0)
            {
    ...
    @catch (NSException* e)
    {
        ...
        if(symbols)
        {
            return [NSString stringWithFormat:@"No diagnosis due to exception %@:\n%@\nPlease file a bug report to the FYCrash project.", e, symbols];
        }
        return [NSString stringWithFormat:@"No diagnosis due to exception %@\nPlease file a bug report to the FYCrash project.", e];
    }
}
